...
--  same as the metal-sysmon / metal-pianobar siblings): dark
...
-- ======================= USER OPTIONS =========================
local SWITCH_FILE = os.getenv('HOME') .. '/.conky/metal-clock/clockswitch'
...
local LED_Y     = 155                     -- (metal-clock-btn, grid 175)
local LED_R     = 6
...
-- family-style flat plate, exactly like metal-sysmon /
...
--  metal-sysmon): a bright metal rim around the clock dial
...
-- launcher: metal-clock, conky-manager, etc.). Guarded so a
...
local function spawn_button()
    local sh = "HB='" .. os.getenv('HOME') .. "'; A=metal-cl; B=ock-btn; if ! pgrep -f 'metal-clock/metal-cl[o]ck-btn' >/dev/null 2>&1; then setsid python3 \"$HB/.conky/metal-clock/$A$B\" </dev/null >>\"$HB/.conky/metal-clock/$A$B.log\" 2>&1 & fi"
    os.execute(sh)
end
