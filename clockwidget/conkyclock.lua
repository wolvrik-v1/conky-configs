...
-- window itself is larger (see conkyclockwdgtrc min/max size))
local DW, DH    = 175, 175
...
-- launcher: conkyclock, conky-manager, etc.). Guarded so a
...
local function spawn_button()
    local sh = "HB='" .. os.getenv('HOME') .. "'; A=btncl; B=ockwdgt; if ! pgrep -f 'clockwidget/btncl[o]ckwdgt' >/dev/null 2>&1; then setsid python3 \"$HB/.conky/clockwidget/$A$B\" </dev/null >>\"$HB/.conky/clockwidget/$A$B.log\" 2>&1 & fi"
    os.execute(sh)
end
